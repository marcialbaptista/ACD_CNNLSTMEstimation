...
def dense_layer(x, size,activation_fn, batch_norm = False,phase=False, drop_out=False, keep_prob=None, scope="fc_layer"):
    ...
    with tf.variable_scope(scope):
        if batch_norm:
            dence_layer = tf.keras.layers.fully_connected(x, size, activation_fn=None)
...
def get_RNNCell(cell_types, keep_prob, state_size, build_with_dropout=True):
    ...
    for cell_type in cell_types:
        if cell_type == 'GRU':
            cell = tf.compat.v1.nn.rnn_cell.GRUCell(num_units=state_size,
                                          bias_initializer=tf.zeros_initializer())  # Or GRU(num_units)
        elif cell_type == 'LSTM':
            cell = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=state_size, use_peepholes=True, state_is_tuple=True,
                                           initializer='glorot_uniform')
        elif cell_type == 'LSTM_LN':
            cell = tf.compat.v1.nn.rnn_cell.LayerNormBasicLSTMCell(state_size)
        elif cell_type == 'GLSTMCell':
            cell = tf.compat.v1.nn.rnn_cell.GLSTMCell(num_units=state_size, initializer='glorot_uniform')
        elif cell_type == 'LSTM_BF':
            cell = tf.compat.v1.nn.rnn_cell.LSTMBlockFusedCell(num_units=state_size, use_peephole=True)
        else:
...
class BatchNorm(object):
    ...
    def __call__(self, x, train=True):
        return tf.keras.layers.batch_norm(x,
                                            decay=self.momentum,
...
def trjectory_generator(x_train, y_train, test_engine_id, sequence_length, graph_batch_size, lower_bound):
    ...
    np.savetxt("Unit.txt", test_engine_id)
