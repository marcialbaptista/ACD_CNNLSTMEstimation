    ...
    with tf.Session() as session:
        ...
        if Train == True:
            ...
            if trj_wise:
                ...
                ax.stem(pred_list, linefmt='g-', label="predicted")
                plt.xlabel('Engine')
                plt.ylabel('RUL')
                plt.legend()
                plt.show()
                ...
                print(y_validation.shape, full_prediction.shape, full_prediction, actual_rul,prediction , "RMSE:", rmse, "MAE:",
                      mae, "Score:", scoring_func(error_list), "R2:", r2)

                if plot:
                    # plt.plot(full_prediction, label="prediction")
                    # plt.plot(actual_rul, label="expected")
                    plt.hist(full_prediction, color='tab:blue')
                    plt.xlabel('RUL error')
