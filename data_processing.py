...
def compute_rul_of_one_file(FD00X, id='engine_id', RUL_FD00X=None):
    ...
    if RUL_FD00X is None:
        ...
        for _id in set(FD00X[id]):
            ...
            rul.extend(range(int(RUL_FD00X.iloc[_id - 1]),0,-1))
            # print(rul)
        return rul
...
def get_PHM08Data(save=False):
    ...
    # print(training_data.shape)get_CMAPSSData
...
def data_augmentation(files=1, low=[10, 40, 90, 170], high=[35, 85, 160, 250], plot=False, combine=False):
    ...
    for i in range(len(high)):
        ...
        if True:
            for id in range(1, total_ids + 1):
                ...
                cutoff_cycle = 20000  ### randomly selecting the cutoff point of the engine cycle
    ...
    train_engine_id = final_train_FD['engine_id']
    engine_ids = train_engine_id.unique()
    total_ids = len(engine_ids)
    for id in range(1, total_ids + 1):
        train_engine_id = final_train_FD['engine_id']
        indexes = train_engine_id[train_engine_id == id].index  ### filter indexes related to id
        final_train_FD.loc[indexes, 'RUL'] = range(len(final_train_FD.loc[indexes, 'cycle']), 0, -1)

    plt.hist(final_train_FD['RUL'].values, color='tab:blue')
    plt.xlabel('RULs')
    plt.ylabel('# of RULs')
    plt.title("raw")
    plt.show()
